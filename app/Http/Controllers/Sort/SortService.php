...
class SortService
{
    ...
    //Worked with postman
    //got json data from postman and pasted them into my json function
    //my postmen response example route
    //http://localhost:8000/api/sortData?prioritize=yes&date=newest&rating=highest&minRating=3
    ...
    public function index(Request $request) {
        ...
        $collection = $collection->where('rating', '>=', $request->input('rating'));

        if ($request->input('prioritize') == 'yes') {
            if ($request->input('rating') == 'highest') {
                if ($request->input('date') == 'newest') {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortByDesc('reviewText')->sortByDesc('rating')->sortByDesc('reviewCreatedOnDate')->all();
                } else {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortByDesc('reviewText')->sortByDesc('rating')->sortBy('reviewCreatedOnDate')->all();
                }
            } else {
                if ($request->input('date') == 'newest') {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortByDesc('reviewText')->sortBy('rating')->sortByDesc('reviewCreatedOnDate')->all();
                } else {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortByDesc('reviewText')->sortBy('rating')->sortBy('reviewCreatedOnDate')->all();
                }
            }
        } else {
            if ($request->input('rating') == 'highest') {
                if ($request->input('date') == 'newest') {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortByDesc('rating')->sortByDesc('reviewCreatedOnDate')->all();
                } else {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortByDesc('rating')->sortBy('reviewCreatedOnDate')->all();
                }
            } else {
                if ($request->input('date') == 'newest') {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortBy('rating')->sortByDesc('reviewCreatedOnDate')->all();
                } else {
                    $collection = $collection->where('rating', '>=', $request->input('minRating'))->sortBy('rating')->sortBy('reviewCreatedOnDate')->all();
                }
            }
        }
